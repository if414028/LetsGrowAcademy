        ...
        <div class="mt-6 overflow-hidden rounded-2xl border bg-white">
            ...
            <div class="border-b px-4">
                <nav class="-mb-px flex gap-2 overflow-x-auto py-3">
                    {{-- Semua --}}
                    <a
                        href="{{ route('sales-orders.index', array_filter(array_merge($baseQuery, ['status' => null]), fn($v) => $v !== null && $v !== '')) }}"
                    ...
                    @foreach(($statuses ?? []) as $st)
                        <a
                            href="{{ route('sales-orders.index', array_filter(array_merge($baseQuery, ['status' => $st]), fn($v) => $v !== null && $v !== '')) }}"
                            class="whitespace-nowrap rounded-xl px-3 py-2 text-sm font-semibold
                                {{ $active === $st
                                    ? 'bg-blue-50 text-blue-700'
                                    : 'text-gray-600 hover:bg-gray-50 hover:text-gray-900' }}"
