    ...
    <div class="mx-auto max-w-6xl">
        ...
        <section class="mt-5 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm sm:p-8">
            <div class="flex flex-col gap-5 sm:flex-row sm:items-center sm:justify-between">
                <div class="flex items-start gap-4"><span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl {{ $styles['icon'] }}"><svg class="h-7 w-7" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8"><path d="M3 7a2 2 0 0 1 2-2h5l2 2h7a2 2 0 0 1 2 2v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V7Z"/></svg></span><div><span class="inline-flex rounded-full px-2.5 py-1 text-[10px] font-extrabold uppercase tracking-wide {{ $styles['badge'] }}">{{ count($category['documents']) }} dokumen</span><h1 class="mt-2 text-3xl font-extrabold tracking-tight text-slate-950">{{ $category['name'] }}</h1><p class="mt-2 max-w-2xl text-sm font-medium leading-6 text-slate-500">{{ $category['description'] }}</p></div></div>
        ...
        <section class="mt-6 grid gap-4 lg:grid-cols-2" aria-label="Dokumen {{ $category['name'] }}">
            @foreach ($category['documents'] as $document)
                <article class="flex flex-col rounded-3xl border border-slate-200 bg-white p-6 shadow-sm transition duration-200 hover:border-slate-300 hover:shadow-lg">
                    <div class="flex items-start gap-4"><span class="flex h-14 w-14 shrink-0 items-center justify-center rounded-2xl bg-red-50 text-xs font-black text-red-600">PDF</span><div class="min-w-0"><h2 class="text-lg font-extrabold text-slate-950">{{ $document['title'] }}</h2><p class="mt-1 text-xs font-bold text-slate-400">{{ $document['pages'] }} halaman · {{ $document['size'] }}</p></div></div>
